  ...
  return (
    <div className="relative flex w-full items-center justify-center overflow-hidden bg-transparent py-20">
      ...
      >
          ...
          {/* Each ring container holds its own visuals and animation logic */}
          <div 
            className="absolute pointer-events-none"
            style={{ ...PATHS.inner.position, width: PATHS.inner.width, height: PATHS.inner.height }}
          >
             <RingVisual pathData={PATHS.inner} />
             <MotionOrbit pathId={`#${PATHS.inner.id}`} duration={40} direction="normal">
          ...
          >
            <RingVisual pathData={PATHS.middle} opacity={0.6} />
             <MotionOrbit pathId={`#${PATHS.middle.id}`} duration={50} direction="reverse">
          ...
          >
            <RingVisual pathData={PATHS.outer} opacity={0.4} />
             <MotionOrbit pathId={`#${PATHS.outer.id}`} duration={60} direction="normal">
...
const MotionOrbit = ({ 
  ...
  children: React.ReactNode 
    ...
    useAnimationFrame((_time, delta) => {
        const moveBy = (direction === 'reverse' ? -1 : 1) * (100 / duration) * (delta / 1000);
        ...
        <div className="absolute inset-0 size-full pointer-events-none">
            {childrenArray.map((child, i) => {
                const itemOffset = useTransform(baseOffset, (v) => {
                    ...
                    const wrappedValue = wrap(0, 100, v + initialPosition);
                    return `${wrappedValue}%`;
                });
...
const RingVisual = ({ pathData, opacity = 1 }: { pathData: typeof PATHS.inner & {id: string}, opacity?: number }) => {
  const gradientId = `${pathData.id}-gradient`;
  return (
    <svg 
      width="100%" 
      height="100%"
    ...
    >
      <defs>
        {/* Define the path here so its geometry is scaled by the SVG's viewBox */}
        <path id={pathData.id} d={pathData.d} />
      ...
      {/* Use the defined path to draw the visible ring */}
      <use href={`#${pathData.id}`} stroke={`url(#${gradientId})`} strokeWidth="1.5" fill="none" />
    </svg>
  );
};